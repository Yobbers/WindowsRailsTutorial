...
### Step 12: Restarting
After finishing your work day and exiting your container (and computer) by using `$ exit` or `CTRL + D` you will need to restart it the next day. Do so by opening your Work Directory within Visual Studio Code by running `$ cd workdirectory && code .`. Then, to make things easier, rename your container by running `$ sudo docker rename old_name_id new_name_id` in the Visual Studio Code terminal. 
Now we are ready to start our newly named container with the command `$ docker start new_name_id` and to enter it right after with `docker exec -it new_name_id bash`. This brings us within our container and gives us the opportunity to restart our Rails server. First enter your application directory by running `# cd myapp` and then finalize by starting the server with `# rails server -p $PORT -b 0.0.0.0`.
